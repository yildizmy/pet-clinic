...
public class UserService {
    ...
    public CommandResponse create(UserRequest request) {
        ...
        final User user = userRepository.save(userRequestMapper.toEntity(request));
    ...
    public CommandResponse update(UserRequest request) {
        final User user = userRepository.findById(request.getId())
                .orElseThrow(() -> new NoSuchElementFoundException(NOT_FOUND_USER));
        // in case username field in the request is not null, set it with current value to prevent update
        request.setUsername(user.getUsername());
        userRepository.save(userRequestMapper.toEntity(request));
